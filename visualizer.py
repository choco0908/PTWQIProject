...
class Visualizer:
    ...
    def prepare(self, char_data):
        ...
        # 차트 1.일봉 차트
        self.axes[ChartIndex.Daily].set_ylabel('Env. ') # y축 레이블 표시
        # 거래량 가시화
        x = np.arange(len(char_data))
        volume = np.array(char_data[:,-1].tolist())
        self.axes[ChartIndex.Daily].bar(x, volume, color='b', alpha=0.3)
        # ohlc란 open, high, low, close의 약자로 이 순서로 구성된 2차원 배열
        ax = self.axes[ChartIndex.Daily].twinx()
    ...
    def plot(self, epoch_str=None, num_epoches=None, epsilon=None, action_list=None, actions=None, num_stocks=None, outvals=None, exps=None, learning=None, initial_balance=None, pvs=None):
        ...
        for actiontype, color in zip(action_list, colors):
            for i in x[actions == actiontype]:
                self.axes[ChartIndex.Agent].axvline(i, color=color, alpha=0.1) # 배경 색으로 행동 표시
        self.axes[ChartIndex.Agent].plot(x, num_stocks, '-k') # 보유 주식 수 그리기

        # 차트 3. 정책 신경망의 출력 및 탐험
        for exp_idx in exps:
            # 탐험을 노란색 배경으로 그리기
            self.axes[ChartIndex.Network].axvline(exp_idx, color='y')
        for idx, outval in zip(x, outvals):
            ...
            self.axes[ChartIndex.Network].axvline(idx, color=color, alpha=0.1)
        styles = ['.r', '.b']
        for action, style in zip(action_list, styles):
            # 정책 신경망의 출력을 빨간색, 파란색 점으로 그리기
            self.axes[ChartIndex.Network].plot(x, outvals[:, action], style)

        # 차트 4. 포트폴리오 가치
        self.axes[ChartIndex.PortfolioValue].axhline(initial_balance, linestyle='-', color='gray')
        self.axes[ChartIndex.PortfolioValue].fill_between(x, pvs, pvs_base, where=pvs > pvs_base, facecolor='r', alpha=0.1)
        self.axes[ChartIndex.PortfolioValue].fill_between(x, pvs, pvs_base, where=pvs < pvs_base, facecolor='b', alpha=0.1)
        self.axes[ChartIndex.PortfolioValue].plot(x, pvs, '-k')
        for learning_idx, delayed_reward in learning:
            # 학습 위치 표시
            if delayed_reward > 0:
                self.axes[ChartIndex.PortfolioValue].axvline(learning_idx, color='r', alpha=0.1)
            else:
                self.axes[ChartIndex.PortfolioValue].axvline(learning_idx, color='b', alpha=0.1)
    ...
    def clear(self, xlim):
        for ax in self.axes[ChartIndex.Agent:]:
            ax.cla() # 그린 차트 지우기
            ax.relim() # limit를 초기화
            ax.autoscale() # 스케일 재설정
        # y축 레이블 재설정
        self.axes[ChartIndex.Agent].set_ylabel('Agent')
        self.axes[ChartIndex.Network].set_ylabel('PG')
        self.axes[ChartIndex.PortfolioValue].set_ylabel('PV')
        for ax in self.axes:
