...
class Visualizer:
    ...
    def plot(self, epoch_str=None, num_epoches=None, epsilon=None, action_list=None, actions=None, num_stocks=None, outvals=None, exps=None, learning=None, initial_balance=None, pvs=None):
        ...
        for actiontype, color in zip(action_list, colors):
            for i in x[actions == actiontype]:
                self.axes[ChartIndex.Agent.value].axvline(i, color=color, alpha=0.1) # 배경 색으로 행동 표시
        self.axes[ChartIndex.Agent.value].plot(x, num_stocks, '-k') # 보유 주식 수 그리기

        # 차트 3. 정책 신경망의 출력 및 탐험
        for exp_idx in exps:
            # 탐험을 노란색 배경으로 그리기
            self.axes[ChartIndex.Network.value].axvline(exp_idx, color='y')
        for idx, outval in zip(x, outvals):
            ...
            self.axes[ChartIndex.Network.value].axvline(idx, color=color, alpha=0.1)
        styles = ['.r', '.b']
        for action, style in zip(action_list, styles):
            # 정책 신경망의 출력을 빨간색, 파란색 점으로 그리기
            self.axes[ChartIndex.Network.value].plot(x, outvals[:, action], style)
        ...
        for learning_idx, delayed_reward in learning:
            # 학습 위치 표시
            if delayed_reward > 0:
                self.axes[ChartIndex.PortfolioValue.value].axvline(learning_idx, color='r', alpha=0.1)
            else:
                self.axes[ChartIndex.PortfolioValue.value].axvline(learning_idx, color='b', alpha=0.1)
    ...
    def clear(self, xlim):
        for ax in self.axes[ChartIndex.Agent.value:]:
            ax.cla() # 그린 차트 지우기
            ax.relim() # limit를 초기화
            ax.autoscale() # 스케일 재설정
        # y축 레이블 재설정
        self.axes[ChartIndex.Agent.value].set_ylabel('Agent')
        self.axes[ChartIndex.Network.value].set_ylabel('PG')
