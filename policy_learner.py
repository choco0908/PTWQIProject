...
class PolicyLearner:
    ...
    def fit(self, num_epoches=1000, max_memory=60, balance=1000000, discount_factor=0, start_epsilon=.5, learning=True):
        logger.info("LR: {lr}, DF: {discount_factor}, TU: [{min_trading_unit}, {max_trading_unit}], DRT: {delayed_reward_threshold}"
                    .format(lr=self.policy_network.lr,
        ...
        for epoch in range(num_epoches):
            ...
            self.visualizer.plot(epoch_str=epoch_str, num_epoches=num_epoches, epsilon=epsilon, action_list=Agent.ACTIONS, actions=memory_action, num_stocks=memory_num_stocks,
                                 outvals=memory_prob, exps=memory_exp_idx, learning=memory_learning_idx, initial_balance=self.agent.initial_balance, pvs=memory_pv)
            ...
            logger.info("[Epoch %s/%s]\tEpsilon:%.4f\t#Expl.:%d/%d\t#Buy:%d\t#Sell:%d\t#Hold:%d\t#Stocks:%d\tPV:%s\tPos:%s\tNEG:%s\tLoss:%10.6f" % (
    ...
    def trade(self, model_path=None, balace=2000000):
        if model_path is None:
            return
        self.policy_network.load_model(model_path=model_path)
        self.fit(balace=balace, num_epoches=1, learning=False)
